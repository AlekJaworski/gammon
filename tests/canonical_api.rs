...
use gammon::family::{
    bernoulli_logit, gaussian_identity, ocat_identity, tweedie_log,
};
...
fn canonical_fit_gaussian_converges() {
    ...
    let canon = gammon::fit(gaussian_identity(), x.view().insert_axis(Axis(1)), y.view(), None, 10)
...
fn canonical_fit_bernoulli_converges() {
    ...
    let canon = gammon::fit(bernoulli_logit(), x.view().insert_axis(Axis(1)), y.view(), None, 10)
...
fn canonical_fit_tweedie_dispatches_to_shape_aware_driver() {
    ...
    let init_p = 1.5;
    let init_phi = 1.0;
    let canon = gammon::fit(tweedie_log(init_p, init_phi), x.view().insert_axis(Axis(1)), y.view(), None, 8)
...
fn error_bernoulli_y_out_of_unit_carries_row() {
    ...
    let err = match gammon::fit(bernoulli_logit(), x.view().insert_axis(Axis(1)), y.view(), None, 4) {
        Ok(_) => panic!("Bernoulli with y=1.5 must error"),
        Err(e) => e,
    };
    let msg = format!("{err}");
    assert!(msg.contains("row 2"), "expected row-index in error, got: {msg}");
    assert!(msg.contains("Bernoulli"), "expected family name in error, got: {msg}");
}
...
fn error_quantile_tau_out_of_range_is_actionable() {
    ...
    let msg = format!("{err}");
    assert!(msg.contains("(0, 1)"), "expected open-interval in error, got: {msg}");
    assert!(msg.contains("tau=1"), "expected offending tau in error, got: {msg}");
}
