...
- **`tests/parity_scat_tf9963.rs` now runs on generated data.** The fixture it
  ...
  a capture of a real adjuster refit: a street address in its `provenance`
  string and 620 sale-derived response values. It is replaced by
...
- **`scat` / `t-dist` numerical results change — every fit, not just edge
  ...
  **If you fit `family="scat"` / `"t-dist"`, expect different smoothing
  ...
  was a real-data capture as shipped in 0.13.0; it was replaced with a
  generated restatement afterwards — see [Unreleased].)
